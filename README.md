...
* **Bounded Recommendation Exploration:** Verified candidates with equivalent metadata fit may vary between refreshes, while stronger prompt/tag matches remain ahead. The web workflow treats the requested limit as a target and backfills model-selected or diversity-filtered results from the remaining verified pool; it returns fewer only when eligible candidates are exhausted.
* **Feedback Before Reinforcement Learning:** Recent recommendation feedback provides a small, age-decayed artist/album ranking signal after exclusions and prompt-fit scoring. Reinforcement learning is intentionally deferred until enough clean feedback exists to evaluate an offline learning-to-rank or contextual-bandit approach safely.
